...
public:
	...
	virtual void AddComment(std::string comment = "") { outData += CMSX::Format("// %s\n", comment.c_str()); }

	virtual void StartSection(std::string name, std::string comment = "")
	{
		outData += CMSX::Format("const u8 %s[] = {", name.c_str()); 
		if (!comment.empty())
			outData += CMSX::Format(" // %s", comment.c_str());
		outData += CMSX::Format("\n");
	}
	virtual void EndSection(std::string comment = "")
	{
	...
	virtual void StartLine() { outData += CMSX::Format("\t"); }
	virtual void AddByte(u8 data)
	{
		std::string Dataformat = ExportData::GetFormat(Data.Size, Data.System) + std::string(", ");
		outData += CMSX::Format(Dataformat.c_str(), data);
		TotalBytes += 1;
	}
	virtual void AddWord(u16 data)
	{
