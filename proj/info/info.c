...
/*
	for(i8 i = 0; i < 4; i++)
	{
		SetPrintPos(8, 24 + i * 8);
		PrintText("Page[");
		PrintInt(i);
		PrintText("] -> Slot ");
		PrintSlot(GetPageSlot(i));
	}

	SetPrintPos(8, 64);
	PrintText("Main-ROM: Slot ");
	PrintSlot(g_EXPTBL[0]);

	SetPrintPos(8, 72);
	PrintText("Sub-ROM:  Slot ");
	PrintSlot(g_EXBRSA);

	SetPrintPos(8, 80);
	PrintText("Disk-ROM: Slot ");
	PrintSlot(g_MASTER);

	for(i8 i = 0; i < 4; i++)
	{
		SetPrintPos(8, 96 + i * 8);
		PrintText("Slot ");
		PrintInt(i);
		PrintText(" ext: ");
		PrintHex8(g_EXPTBL[i]);
	}

	SetPrintPos(8, 136);
	PrintText("Prim:   ");
	PrintHex8(g_PortPrimarySlot);
	...
	SetPrintPos(8, 24);
	PrintText("Keyboard matrix");

	for(i8 i = 0; i < 11; i++)
	{
		SetPrintPos(16, 40 + i * 8);
		PrintText("KB[");
		PrintInt(i);
		PrintText("]: ");
	}
...
/*
		for(i8 i = 0; i < 11; i++)
		{
			SetPrintPos(8 + 8 * 8, 40 + i * 8);
			PrintHex8(g_OLDKEY[i]);
		}
*/
		//VDP_WaitRetrace();
	}
